...
// Nutzt micros() für Auflösung bis in den µs-Bereich.
...
void checkMidiTriggerTimers() {
  ...
  for (int ch = 0; ch < ChMax; ch++) {
    if (gMidiTriggerOffTime[ch] != 0) {
      // Timer abgelaufen? micros()-Overflow ist nach ~70 Minuten,
      // unsigned wrap-around wird durch int32_t-Cast korrekt behandelt.
...
void handleNoteOn(byte channel, byte pitch, byte velocity) {
  ...
  if (state.isRunning) return;
    ...
    // Pulsbreite aus Velocity berechnen (1-127):
  // - Velocity 1   ->    100 µs (kürzester Impuls)
  // - Velocity 127 -> 10000 µs (10 ms, längster Impuls)
  // Achtung: bei aktivem MIDI Monitor werden die Impulse verlängert!
  // Lösung über Timer2 / ISR bracht das Problem, dass viele Midinoten hintereinander verzögert wurden.
  uint32_t pulseUs = (uint32_t)map(velocity, 1, 127, 1, state.pulseWidth); // Max 3-4 ms scheint gut zu sein
  ...
  if (ch == ChOH) {
    ...
    if (pitch == 44) {
      ...
      midiTriggerOnTimed(kHiHatChannel, 2000UL);
    } else {
      // Closed Hi-Hat (Note 42): Impuls mit state.pulseWidth
      midiTriggerOnTimed(kHiHatChannel, pulseUs);
    }
  } else {
